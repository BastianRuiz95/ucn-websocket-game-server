...
export class SendDataUseCase {
  exec(player: Player, data: object): GameResponse {
    const { match } = player;
    this._validateMatch(match);
    const secondPlayer = this._getOpponent(player, match);

    secondPlayer.sendEvent(
  ...
  private _getOpponent(player: Player, match: Match) {
    return player.id === match.senderPlayer.player.id
      ? match.destPlayer.player
      : match.senderPlayer.player;
  }
}
